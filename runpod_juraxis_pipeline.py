...
class JuraxisRunPodPipeline:
    ...
    def extract_legal_metadata(self, text: str, case_name: str = "") -> Dict[str, Any]:
        ...
        # Sanitize input text to prevent regex errors
        if not text:
            return metadata
        
        # Escape regex special characters in the input text
        text_safe = re.escape(text.lower())
        ...
        # Extract legal areas with defensive regex handling
        for area, patterns in self.legal_area_patterns.items():
            for pattern in patterns:
                try:
                    ...
                    logger.warning(f"Regex error in legal area pattern '{pattern}': {e}")
                    continue
        ...
        # Extract citations with defensive regex handling
        citations = []
        for pattern in self.compiled_citation_patterns:
            try:
                citations.extend(pattern.findall(text))
            except re.error as e:
                logger.warning(f"Regex error in citation pattern: {e}")
                continue
    ...
    def process_legal_document(self, doc_data: Dict[str, Any]) -> Optional[LegalDocument]:
        ...
        try:
            ...
            case_name = self.sanitize_text_input(doc_data.get('case_name', 'Unknown Case'))
            citation = self.sanitize_text_input(doc_data.get('citation', ''))
            court = self.sanitize_text_input(doc_data.get('court', 'Unknown Court'))
            ...
            # Extract legal metadata with additional error handling
            try:
                metadata = self.extract_legal_metadata(plain_text, case_name)
            except Exception as e:
                logger.warning(f"Failed to extract metadata for {case_name}: {e}")
            ...
            # Enhanced error logging to help debug regex issues
            case_name = doc_data.get('case_name', 'Unknown')
            logger.error(f"Failed to process document {case_name}: {e}")
            
            # If it's a regex error, log the problematic document data for debugging
            if "unterminated character set" in str(e) or "bad character" in str(e):
                logger.error(f"REGEX ERROR DEBUG - Document causing regex failure:")
                logger.error(f"  case_name: {repr(case_name)}")
                logger.error(f"  citation: {repr(doc_data.get('citation', ''))}")
                logger.error(f"  court: {repr(doc_data.get('court', ''))}")
                logger.error(f"  plain_text (first 200 chars): {repr(doc_data.get('plain_text', '')[:200])}")
                logger.error(f"  full_text (first 200 chars): {repr(doc_data.get('full_text', '')[:200])}")
                
                # Save problematic document to file for analysis
                debug_file = self.output_dir / f"debug_regex_error_{int(time.time())}.json"
                try:
                    with open(debug_file, 'w', encoding='utf-8') as f:
                        json.dump(doc_data, f, indent=2, ensure_ascii=False)
                    logger.error(f"  Saved problematic document to: {debug_file}")
                except Exception as save_e:
                    logger.error(f"  Could not save debug file: {save_e}")
    ...
    def sanitize_text_input(self, text: str) -> str:
        """Sanitize text input to prevent regex errors and clean problematic characters"""
        if not text:
            return ""
        
        # Remove or replace problematic characters that can break regex
        # Keep the text readable but safe for regex processing
        text = str(text)  # Ensure it's a string
        
        # Replace problematic regex characters with safe equivalents
        # Be more aggressive about removing special characters
        text = text.replace('[', '(').replace(']', ')')
        text = text.replace('{', '(').replace('}', ')')
        text = text.replace('\\', '/')  # Replace backslashes
        text = text.replace('|', '-')   # Replace pipes
        text = text.replace('^', ' ')   # Replace carets
        text = text.replace('$', ' ')   # Replace dollar signs
        text = text.replace('*', ' ')   # Replace asterisks
        text = text.replace('+', ' ')   # Replace plus signs
        text = text.replace('?', ' ')   # Replace question marks
        
        # Keep only truly safe characters - be very restrictive for regex safety
    ...
    def process_csv_bz2_file(self, file_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Robust BZ2 file handling with proper encoding
            with bz2.BZ2File(file_path, 'rb') as compressed_file:
                with io.TextIOWrapper(compressed_file, encoding='utf-8') as f:
    ...
    def convert_court_row_to_document(self, row: Dict[str, str]) -> Dict[str, Any]:
        ...
        # Sanitize CSV input to prevent regex special characters
        full_name = self.sanitize_text_input(row.get('full_name', 'Unknown Court'))
        jurisdiction = self.sanitize_text_input(row.get('jurisdiction', 'unknown'))
        court_id = self.sanitize_text_input(row.get('id', ''))
        court_url = self.sanitize_text_input(row.get('url', ''))
        
        return {
            'id': court_id,
            'case_name': f"Court Information: {full_name}",
            'citation': court_id,
            'court': full_name,
            'court_level': self.determine_court_level(full_name),
            'jurisdiction': jurisdiction,
            'date_decided': '',
            'full_text': f"Court: {full_name}\nJurisdiction: {jurisdiction}\nURL: {court_url}",
            'plain_text': f"This is {full_name} in {jurisdiction}.",
            'legal_areas': ['court_administration'],
            'authority_weight': self.calculate_court_authority_weight(full_name),
    ...
    def convert_citation_row_to_document(self, row: Dict[str, str]) -> Dict[str, Any]:
        ...
        # Sanitize CSV input to prevent regex special characters
        citing_opinion = self.sanitize_text_input(row.get('citing_opinion', 'Unknown'))
        cited_opinion = self.sanitize_text_input(row.get('cited_opinion', 'Unknown'))
        citation_id = self.sanitize_text_input(row.get('id', ''))
        
        return {
            'id': f"citation_{citation_id}",
            'case_name': f"Citation Network: {citing_opinion} → {cited_opinion}",
            'citation': f"Citation ID: {citation_id}",
            'court': 'Citation Database',
            'court_level': 'database',
            'jurisdiction': 'multi_jurisdiction',
            'date_decided': '',
            'full_text': f"Citation relationship: {citing_opinion} cites {cited_opinion}",
            'plain_text': f"This citation links opinion {citing_opinion} to {cited_opinion}.",
            'legal_areas': ['citation_analysis'],
            'authority_weight': 0.5,
            'binding_jurisdictions': ['multi_jurisdiction'],
            'citations_extracted': [cited_opinion],
            'citations_validated': []
        }
